...
function displayCard(datatec) {
    var tarjetasHtml3 = ""

    for (var i = 0; i < datatec.length; i++) {
        if(data.currentDate> datatec[i].date){
        tarjetasHtml3 += `

        <div class="card p-3 bg-dark text-light mb-5" style="width: 18rem;">       
        <div class="card-body">        
        <img class="card-img-top" src="${datatec[i].image}"></div>        
        <div class=" text-center dataCard">        
        <h5 class="card-title text-center mt-2">${datatec[i].name}</h5>        
        <p class="card-text text-center ">${datatec[i].description}</p>
        <p>${datatec[i].date}</p>        
        <p class="card-text text-center ">Price: $ ${datatec[i].price}</p>        
        <buttom class="btn btn-danger"><a class=" text-light text-decoration-none" href="cardata.html?id=${datatec[i]._id}">¡ See more !</a></buttom>        
        </div>        
        </div>
        `          
    }
...
if(tarjetasHtml3 === ''){
    tarjetasHtml3 = `<h3 class="nofound text-light d-flex justify-content-center p-5 m-5"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-report-search" width="40" height="40" viewBox="0 0 24 24" stroke-width="1.5" stroke="#fd0061" fill="none" stroke-linecap="round" stroke-linejoin="round">
    ...
    document.querySelector('#pastcomCards').innerHTML = tarjetasHtml3  
}
